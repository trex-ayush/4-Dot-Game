...
exports.saveCompletedGame = async (gameData) => {
  try {
    const game = new Game({
      gameId: gameData.gameId,
      player1: gameData.players[1].username,
      player2: gameData.players[2].username,
      winner: gameData.winner ? gameData.players[gameData.winner].username : null,
      result: gameData.result,
      moves: gameData.moves,
      isAgainstBot: gameData.isAgainstBot,
...
exports.updateUserStats = async (gameData) => {
  try {
    ...
    if (gameData.originalPlayer1 && gameData.originalPlayer1 !== 'BOT') {
      ...
      await User.findOneAndUpdate(
        { username: gameData.originalPlayer1 },
        updateData,
        { upsert: true, new: true }
      );
    }
    ...
    if (gameData.originalPlayer2 && gameData.originalPlayer2 !== 'BOT') {
      let updateData = { $inc: { gamesPlayed: 1, losses: 1 } }; // Count as loss if timed out
      await User.findOneAndUpdate(
        { username: gameData.originalPlayer2 },
        updateData,
        { upsert: true, new: true }
      );
    }
